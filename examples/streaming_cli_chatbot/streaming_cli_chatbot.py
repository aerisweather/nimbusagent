...
INIT_PROMPT = (
    f"Hello I am {AGENT_NAME} a helpful assistant. Let's chat, ask me a question."
)
...
def initialize_agent():
    ...
    sys_prompt = (
        f"You are {AGENT_NAME}, a useful assistant that likes to chat\n"
...
def interact_with_chatbot(agent, user_input):
    ...
    for content in agent.ask(user_input):
        if not received_response:
            spinner.stop()
            sys.stdout.write(
                f"{CURSORS['show']}{COLORS['bold']}{COLORS['blue']}{AGENT_NAME}:{COLORS['reset']} "
            )
...
def main():
    ...
    while True:
        user_input = input(
            f"{CURSORS['block']}{COLORS['bold']}{COLORS['orange']}You:{COLORS['reset']} "
        ).strip()
